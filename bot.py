...
# ========== دالة بناء روابط الصفحات ==========
def build_page_urls(base_url, category_type, max_pages):
    """بناء روابط الصفحات المختلفة حسب نوع القسم"""
    urls = []
    ...
    for page in range(1, max_pages + 1):
        ...
        if 'gp/bestsellers' in base_url or 'gp/goldbox' in base_url:
            # Best sellers & Goldbox: ?pg=2
            separator = '&' if '?' in base_url else '?'
            urls.append((f"{base_url}{separator}pg={page}", page))
            
        elif '/s?' in base_url or 'keywords=' in base_url:
            # Search results: &page=2
            separator = '&' if '?' in base_url else '?'
            urls.append((f"{base_url}{separator}page={page}", page))
            
        elif 'deals' in base_url or 'warehouse-deals' in base_url:
            # Deals: &page=2
            separator = '&' if '?' in base_url else '?'
            urls.append((f"{base_url}{separator}page={page}", page))
            
        elif 'outlet' in base_url:
            # Outlet: &page=2
            separator = '&' if '?' in base_url else '?'
            urls.append((f"{base_url}{separator}page={page}", page))
            
        elif 'gp/coupons' in base_url:
            # Coupons: &page=2
            separator = '&' if '?' in base_url else '?'
            urls.append((f"{base_url}{separator}page={page}", page))
            
        elif 'todays-deals' in base_url or 'lightning' in base_url:
            # Today deals: &page=2
            separator = '&' if '?' in base_url else '?'
            urls.append((f"{base_url}{separator}page={page}", page))
...
# ========== البحث مع الترقيم الصفحي ==========
def search_all_deals(chat_id, status_message_id):
    all_deals = []
    session = create_session()
    
    # ✅ 200+ قسم شامل مع عدد الصفحات لكل قسم
    categories = [
        # 🏆 Best Sellers الأساسية (3 صفحات لكل قسم)
        ...
        # 🔥 عروض مخفية - Warehouse Deals (2 صفحات)
        ...
        # 🔥 عروض سرية - Clearance (3 صفحات للحصول على أكبر عدد)
    ...
    total_pages = sum(PAGES_CONFIG.get(cat[2], 1) for cat in categories)
    processed_pages = 0
    
    for idx, (base_url, cat_name, cat_type) in enumerate(categories, 1):
        try:
            max_pages = PAGES_CONFIG.get(cat_type, 1)
            page_urls = build_page_urls(base_url, cat_type, max_pages)
            
            for page_url, page_num in page_urls:
                processed_pages += 1
                
                # تحديث الحالة كل 5 صفحات
                if processed_pages % 5 == 0:
                    progress = f"⏳ جاري البحث... ({processed_pages}/{total_pages} صفحة)\n📍 {cat_name} - صفحة {page_num}"
                    try:
                ...
                logger.info(f"🔍 [{cat_name}] Page {page_num}/{max_pages}")
                ...
                logger.info(f"   Found {len(items)} items in page {page_num}")
                
                for item in items:
                    try:
                        deal = parse_item(item, cat_name, 'best_sellers' in cat_type)
                        if deal:
    ...
    logger.info(f"✅ Total: {len(all_deals)} from {processed_pages} pages")
    return all_deals
...
def hi_cmd(update: Update, context: CallbackContext):
    ...
    status_msg = update.message.reply_text("🔍 *بدأت البحث في 500+ صفحة...*\n⏱️ 10-15 دقيقة", parse_mode='Markdown')
